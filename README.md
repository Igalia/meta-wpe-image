...
1. Build your target enviroment. Examples:
   ...
   * RPI3:
     ...
     kas-container build kas/demos-image-raspberrypi3.yml:kas/common/wpe-nightly.yml
     ```
   ...
   * RPI4-64 using the propietary graphics stack (vc4graphics):
     ...
     mkdir -p ${KAS_WORK_DIR}
     kas-container build kas/demos-image-raspberrypi4-64.yml:kas/common/wpe-nightly.yml
     ```
   ...
   * RPI5:
     ...
     mkdir -p ${KAS_WORK_DIR}
     kas-container build kas/demos-image-raspberrypi5.yml:kas/common/wpe-nightly.yml
     ```
...
1. Preparing the SDK Toolchain
   ...
   kas-container shell kas/demos-image-raspberrypi3.yml -c "bitbake ${BASE_IMAGE} -c populate_sdk"
   ```

   Check the resulting SDK installer:

   ```sh
   ls ${KAS_WORK_DIR}/build/tmp/deploy/sdk/demos-*-raspberrypi3-toolchain-*.sh
   ```
...
2. Installing the SDK Toolchain
   ...
   ./tmp/deploy/sdk/demos-*-raspberrypi3-toolchain-*.sh -d ~/toolchain_env -y
   ```
